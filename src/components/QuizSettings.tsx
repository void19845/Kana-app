...
// ─── Types (inchangés, re-exportés pour QuizMode) ─────────────────────────────
...
export function QuizSettings() {
    // Séance 4 : lit le context directement, plus besoin de props
    ...
    return (
            ...
            <div className="sb-section">
                ...
                <div className="chip-group">
                    {Object.keys(PRESETS).map(k => (
                        <Chip key={k} label={k.charAt(0).toUpperCase() + k.slice(1)}
                              active={false}
                              onClick={() => dispatch({
                                  type: 'SET_PRESET',
                                  payload: new Set(PRESETS[k]),
                              })} />
                    ))}
                </div>
            </div>
        ...
        <span className="sb-label">
          Lignes ({settings.activeRows.size}/{KANA_ROWS.length})
        </span>
