...
import type { StatsStore } from '../hooks/useStats';
...
export function StatsView() {
  ...
  const hardList = Object.values(charErrors).sort((a, b) => b.count - a.count).slice(0, 15);
  ...
  return (
    <div className="stats-view">
              ...
              {sessions.map((s, i) => {
                ...
                return (
                  <li key={i} className="session-row">
                    ...
                    <span className="session-script">{s.script === 'hiragana' ? 'ひ' : 'カ'}</span>
                    <span className="session-score">{s.correct}<span className="session-total"> / {s.total}</span></span>
      ...
      <div className="stat-card stat-card--full">
              ...
              {hardList.map(c => (
                <div key={c.char} className="hard-item">
                  ...
                  <div className="hard-bar">
                    <div className="hard-fill" style={{ width: `${Math.round((c.count / maxErr) * 100)}%` }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        {sessions.length > 0 && (
          <button className="clear-btn" onClick={handleClear}>Effacer les statistiques</button>
        )}
      </div>
    </div>
  );
