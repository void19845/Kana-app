...
export function QuizSummary({ score, errors, script, onRestart, onStudy }: QuizSummaryProps) {
  const pct   = Math.round((score.correct / score.total) * 100);
  const grade = pct >= 90 ? '優' : pct >= 70 ? '良' : pct >= 50 ? '可' : '不';
  const label = pct >= 90 ? 'Excellent !'
              : pct >= 70 ? 'Bien joué.'
              : pct >= 50 ? 'Continuez !'
              : 'À réviser…';

  // Dédoublonner les erreurs par romanji+hiragana
      ...
      <div className="summary-box">
        ...
        {uniqueErrors.length > 0 && (
          <div className="summary-errors">
            <p className="summary-errors-title">
              À revoir ({uniqueErrors.length})
            </p>
        ...
        <div className="summary-actions">
          <button className="summary-btn summary-btn--primary" onClick={onRestart}>
            Recommencer
          </button>
          <button className="summary-btn summary-btn--ghost" onClick={onStudy}>
            Réviser la table
          </button>
        </div>
